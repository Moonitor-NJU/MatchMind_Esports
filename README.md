...
- 项目类型：实时电竞赛程整理与晋级形势分析 Agent
- 展示方式：运行网页 Demo，不需要 PPT
- 入口文件：`server.js`
- 前端页面：`public/index.html`
- 本地运行：`npm start` 或 `node server.js`
- 访问地址：`http://localhost:3000`
- GitHub 提交内容：源码、前端页面、本地兜底数据、规则配置与 README
- 不提交内容：`.env`、API Key、临时图片素材和本机缓存

本项目的重点不是简单把赛程丢给大模型，而是先由后端把实时赛程、签表、规则、新闻、阵容和单场检索证据整理成结构化上下文，再交给 AI 输出可解释的观赛结论。
...
- 对未开赛比赛，后端会为每场比赛生成单独的 matchup research。
- 检索内容包括：赛前前瞻、赔率/市场信息、虎扑/PTT/知乎等社区讨论、近期状态、阵容和首发信息。
- AI 预测卡不再只根据 `0-0`、名单顺序或本地热度机械输出 `51% vs 49%`。
- 模型会根据当前证据给出胜率、看好方、关键因素和置信度；证据不足时会标注为“低置信”或“悬念局”。
- 搜索结果会被分类为 `odds-market`、`community-discussion`、`preview-opinion`、`roster-context` 等，避免把舆论当作官方事实。
...
# 单场赛前预测检索缓存，默认 30 分钟
MATCHUP_RESEARCH_CACHE_TTL_MS=1800000
...
| `/api/prediction-card` | POST | 未进行比赛的结构化胜率预测卡 |
...
- **赔率、社区和前瞻文章**：用于赛前预测的外部语境，不作为官方赛制依据。
...
1. 打开首页，说明项目会自动加载当前热门英雄联盟赛事。
2. 切换 MSI、EWC、LPL、LCK 等赛事，展示新闻、赛程和焦点会随赛事变化。
3. 对比常规赛/小组赛的赛程名单与季后赛的签表晋级视图。
4. 选择一场未赛比赛，展示 AI 预测卡的胜率、置信度和关键因素。
5. 使用 AI 问答询问“这场比赛为什么重要”“哪支队伍更值得看”。
6. 简要说明后端如何结合 PandaScore、规则配置、新闻检索、阵容证据和大模型。
...
- 页面可以正常打开。
- 赛事选择列表有当前赛事。
- 新闻轮播、赛程、晋级形势、AI 预测和 AI 问答均能显示。
- 未配置 API 或接口失败时，页面会显示本地兜底数据，而不是崩溃。
