...
首页会通过 `/api/news` 拉取赛事新闻，解析标题、外链、发布时间，并尽量从 RSS、文章 OpenGraph 或 LoL Esports 官方新闻页中提取封面图，做成类似 B 站主推位的可点击轮播。默认新闻源优先使用 LoL Esports 官方新闻页；如果你的网络环境访问不稳定，可以在 `.env` 中替换为你能访问的 RSS/新闻页：

```bash
# 多个 RSS 用英文逗号分隔
NEWS_FEEDS=https://lolesports.com/en-US/news,https://www.dexerto.com/league-of-legends/feed/
...
赛区焦点会同时参考赛制压力和观众热度：系统只内置主流队伍的别名和基础关注度，不再写死“黑马/豪门/情怀”等队伍人设；这些标签会根据当前赛果、签表位置、新闻标题命中和近期是否完成下克上动态生成。因此焦点不只按下一场比赛排序，也会随着新闻、赛果和淘汰赛路径变化，把真正的生死战、爆冷线索、流量对撞和资格赛路径提前。
